function [saida]=vadsohnGEN48(xs,si,vecGabarito,nFalas)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ainf=xs(1); %ainf  =; %tortuosity
% phi=xs(2);  %phi   =; %porosity
% sigma=xs(3);%sigma =; %flow resistivity
% sigmap=xs(4);
% theta1=xs(5);
% theta2=xs(6);
...
%     qq.pr = xs(1);
    ...
    qq.ts = xs(8);
    qq.tn = xs(9);
    qq.xn= xs(10);        % minimum prior SNR = -Inf dB
    ...
    % qq.xn=40;        % minimum prior SNR = -Inf dB
    %     qq.ne=0;          % noise estimation: 0=min statistics, 1=MMSE [0]
    
    
    
    % %     qq.of=of;       %2;       % overlap factor = (fft length)/(frame increment)
    % %     qq.pr=0.7;      % Speech probability threshold
    %     qq.ts=0.1;      % mean talkspurt length (100 ms)
    %     qq.tn=0.05;     % mean silence length (50 ms)
    % %     qq.ti= ti;     %10e-3;    % desired output frame increment (10 ms)
    % %     qq.tj= tj;     %10e-3;    % internal frame increment (10 ms)
    %     qq.ri=0;        % round ni to the nearest power of 2
    % %     qq.ta=ta;      %0.396;    % Time const for smoothing SNR estimate = -tinc/log(0.98) from [2]
    %     qq.gx=1000;     % maximum posterior SNR = 30dB
    %     qq.gz=1e-4;     % minimum posterior SNR = -40dB
...
%% Erros
%Descrição dos erros e ponderações...

for iSample = 1:length(vs)-1
    if vs(iSample)     == 0 && vs(iSample+1) == 1
        countTalk(iSample+1) = 1;
    elseif vs(iSample) == 0 && vs(iSample+1) == 0
        countTalk(iSample) = 0;
    elseif vs(iSample) == 1 && vs(iSample+1) == 0
        countTalk(iSample) = 1;
    elseif vs(iSample) == 1 && vs(iSample+1) == 1
        countTalk(iSample) = 0;
    end
end
nSpeeches = ceil(sum(countTalk)/2);

if nSpeeches==0
   nSpeeches = 1000e9;
end
%da pra melhorar os pesos aqui
if nSpeeches>=nFalas
    ratioSpeeches = nSpeeches/nFalas;
elseif nSpeeches<nFalas
    ratioSpeeches = (nFalas/nSpeeches);
end


% switch nSpeeches
%      case nSpeeches>=nFalas*11
%     ratioSpeeches = nSpeeches/nFalas*11;
%     case nSpeeches==nFalas*10
%     ratioSpeeches = nSpeeches/nFalas*10;
%  case nSpeeches==nFalas*9
%     ratioSpeeches = nSpeeches/nFalas*9;
%        case nSpeeches>nFalas*8
%     ratioSpeeches = nSpeeches/nFalas*8;
%  case nSpeeches==nFalas*7
%     ratioSpeeches = nSpeeches/nFalas*7;
%  case nSpeeches==nFalas*6
%     ratioSpeeches = nSpeeches/nFalas*6;
%        case nSpeeches==nFalas*5
%     ratioSpeeches = nSpeeches/nFalas*5;
%  case nSpeeches==nFalas*4
%     ratioSpeeches = nSpeeches/nFalas*4;
%  case nSpeeches==nFalas*3
%     ratioSpeeches = nSpeeches/nFalas*3;
%        case nSpeeches==nFalas*2
%     ratioSpeeches = nSpeeches/nFalas*2;
%  case nSpeeches==nFalas*1
%     ratioSpeeches = nSpeeches/nFalas*1;
%     
%   
% %Melhorar aqui
%    
%  case nSpeeches<nFalas
%     ratioSpeeches = (nSpeeches/nFalas)*3;
%   
% end

% disp('%i',ratioSpeeches)
vetErro_Sohn_48k = ((vecGabarito')-(vs));


for i = 1:length(vetErro_Sohn_48k)
    
    erro_pos_Sohn_48k(i) = sum(1==vetErro_Sohn_48k(i));
    erro_neg_Sohn_48k(i) = abs(sum(-1==vetErro_Sohn_48k(i)));
    
end


% saida = (((sum(erro_neg_Sohn_48k)) + ...
%     (1.5*sum(erro_pos_Sohn_48k)))*(ratioSpeeches^2))/length(vs);

saida = (((sum(erro_neg_Sohn_48k)) + ...
    (1.5*sum(erro_pos_Sohn_48k))))/length(vs);

% saida = sum(transpose(vs)~=vecGabarito)
