function [saida]=vadsohnGEN(xs,si,vecGabarito,nFalas)
...
%% Erros
    %Descri??o dos erros e pondera??es...
       vetErro_Sohn_8k = vecGabarito'-(vs);
   countTalk = zeros(1,length(vs));
...
elseif nSpeeches<nFalas
    ...
    for i = 1:length(vetErro_Sohn_8k)
        
        erro_pos_Sohn_8k(i) = sum(1==vetErro_Sohn_8k(i));
        erro_neg_Sohn_8k(i) = abs(sum(-1==vetErro_Sohn_8k(i)));
           
    end
    
saida = ((sum(erro_neg_Sohn_8k) + (1.5*sum(erro_pos_Sohn_8k))))/length(vs);

%saida = ((sum(erro_neg_Sohn_8k) + (1.5*sum(erro_pos_Sohn_8k)))*(ratioSpeeches^2))/length(vs);
