...
% Native Implementation Without ITA Toolbox
...
function playPlain(signal_run,nCh)
    % Initialize playback with default soundcard (-1) using playrec
    if playrec('isInitialised')==1
        playrec('reset')
    end
    
    % Initialize interface, -1 implies default device
    playrec('init', signal_run.samplingRate, -1, -1);
    
    if playrec('isInitialised')
        disp('Playing 24-channel output...');
        playrec('play', signal_run.time, 1:nCh);
    else
        disp('Warning: Soundcard not initialized. Make sure Audio APIs are supported.');
    end
end
